...
def get_internships():
    ...
    try:
        ...
        # Auto-filter expired internships (past deadline)
        from datetime import date
        today = date.today()
        query = query.filter(
            db.or_(
                Internship.application_deadline == None,  # no deadline set = always show
                Internship.application_deadline >= today   # deadline not passed yet
            )
        )
