...
Every recommendation includes a human-readable `explanation` object generated by `_explain_match()` inside `HybridMatcher`. This makes the AI's reasoning transparent to students.
...
"explanation": {
  ...
  "major_match": true,
  "top_keywords": ["model", "research", "analysis"],
  "reasons": [
    "Your skills match 2 required skills for this role.",
    "Your major aligns with the target field.",
    "Your interests overlap with this role's domain."
  ]
}
```
...
| Field | Description |
|---|---|
| `matched_skills` | Intersection of student skills and internship required skills |
| `matched_interests` | Student interests that overlap with internship domain |
| `major_match` | Whether student's major aligns with the internship's target major |
| `top_keywords` | Top TF-IDF keywords shared between student profile and internship text |
| `reasons` | Bullet-point human-readable explanations shown in the dashboard UI |
  ...
  "recommendations": [
    {
      ...
      "match_details": {
        ...
        "explanation": {
          ...
          "major_match": true,
          "top_keywords": ["model", "data"],
          "reasons": ["Your skills match 1 required skill for this role."]
        }
      }
    },
    ...
...
Results are cached client-side so they survive page refresh without re-charging points:

- **Web:** stored in `localStorage` under key `rec_{userId}`. Loaded instantly on page mount.
- **Mobile:** stored in `AsyncStorage` under key `rec_{userId}`. Restored asynchronously on mount.

The cache is only cleared when the student explicitly requests new recommendations via the **↻ New** button in the dashboard header (this triggers a fresh fetch and charges 10 points again).
...
> **Recommendation persistence:** AI recommendation results are cached in `localStorage` (web) and `AsyncStorage` (mobile) so refreshing the page does **not** charge points again. The cache is only cleared when the student explicitly presses **↻ New** to fetch fresh recommendations.
...
| Home | Dashboard with stats, Top AI Picks, and quick actions |
...
The mobile dashboard (`DashboardScreen.tsx`) mirrors the web dashboard with:

- **Top AI Picks section**: top 3 AI-ranked internships shown above Quick Actions, loaded instantly from `AsyncStorage` cache.
- **AI Recommendations section**: full recommendation list with XAI explanation panels (matched skill chips, bullet reasons, score bars, Major Match / Interest Fit badges).
- **Rank badges**: 🥇 Gold (#1), 🥈 Silver (#2), 🥉 Bronze (#3), neutral for #4+.
- **↻ New button**: appears in the AI Recommendations header when results are loaded; clears cache and re-fetches (charges 10 points).
...
The student dashboard (`src/pages/Dashboard.tsx`) has four tabs:

| Tab | Description |
|---|---|
| Overview | Stats cards, Top AI Picks (top 3 cached recommendations), quick links |
| Recommended | Full AI recommendation list with XAI explanations, score bars, skill tags, and rank badges |
| Applications | Application history with status badges |
| Profile | Profile editor, CV builder status, resume upload |

**Top AI Picks (Overview tab):** Shows the top 3 AI-ranked internships at a glance. Loaded instantly from cache — no points charged on refresh. A **↻ New** button in the Recommended tab header triggers a fresh fetch (10 pts).

**Rank badges:** Each recommendation card shows a medal badge:
- 🥇 **#1** — Gold
- 🥈 **#2** — Silver  
- 🥉 **#3** — Bronze
- **#4+** — Neutral (theme-adaptive)

**XAI panel (Recommended tab):** Each card shows:
- Matched skill tags (green `✓ skill`)
- Human-readable bullet reasons
- Semantic (70%) and Keyword (30%) score progress bars
- *Major Match* and *Interest Fit* badges where applicable
